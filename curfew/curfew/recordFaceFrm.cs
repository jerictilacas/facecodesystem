...
namespace curfew
{
    public partial class recordFaceFrm : Form
    {
        ...
        public recordFaceFrm()
        {
            InitializeComponent();
            nameLbl.Text = fname + " " + lname;
            Face = new CascadeClassifier(Application.StartupPath + "/Cascade/haarcascade_frontalface_alt2.xml");
            ...
            if (currentFrame != null)
            {
                ...
                if (RECORD && facesDetected.Length > 0 && resultImages.Count < num_faces_to_aquire)
                {
                    ...
                    if (resultImages.Count == num_faces_to_aquire)
                    {
                        ...
                        Application.Idle -= new EventHandler(FrameGrabber);
                        ADD_ALL_Click();
                        MessageBox.Show("Successfully Added Image!");
                    }
                }
        ...
        private bool save_training_data(Image face_data)
        {
            try
            {
                ...
                string facename = "face_" + nameLbl.Text.Trim().ToLower() + "_" + rand.Next().ToString() + ".jpg";
                while (file_create)
                {
                    ...
                    {
                        facename = "face_" + nameLbl.Text.Trim().ToLower() + "_" + rand.Next().ToString() + ".jpg";
                    }
                }
                ...
                else
                {
                    ...
                    using (XmlWriter writer = XmlWriter.Create(FS_Face))
                    {
                        ...
                        writer.WriteElementString("NAME", nameLbl.Text);
        ...
        private void recordFaceFrm_FormClosed(object sender, FormClosedEventArgs e)
        {
            grabber.Stop();
            grabber.Stop();
            adduc.checkVal = true;
